...
if ($insert_stmt = $mysqli->prepare("insert into ddTransactions(email, appno, ddno, bank_name, bank_branch,amount, issue_date) values(?,?,?,?,?,?,?) ")) {   	
	//amount column added ...by RB on Dec 9,2017		
	$insert_stmt->bind_param('sssssss', $email, $appno, $ddno, $bank_name, $bank_branch,$amount, $issue_date); 
	$insert_stmt->execute();
	$insert_stmt->close();
