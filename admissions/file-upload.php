<?php include('backendaccesscheck.php');
 ...
 function validateAndSave($file) {
		...
		)
	 {
		...
		$image_ext_name = preg_replace('/[^\w\.\- ]/', '', $_SESSION["email"]);
		$save_path = preg_replace('/\.(.+)$/' , '', $directory).$image_ext_name;
		  ...
		  )
		{
			...
			$result['message'] = 'Unable to save file!'.$save_path."==".$file["type"];
				...
				{   			
					$insert_stmt->bind_param('ss', $profile_image_name, $_SESSION['email']); 
					$insert_stmt->execute();
					$insert_stmt->close();
					...
					if($_SESSION['photo_status']!="yes")
					{
						...
						{   			
							$insert_stmt->bind_param('ss', $photo_status, $_SESSION['email'] ); 
							$insert_stmt->execute();
							$insert_stmt->close();
						}
