...
else
{
	...
	header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?errorMsg=1.Payment Failed.Please try again.");
...
else
{
	...
	header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?errorMsg=2.Payment Failed.Please try again.");
...
if($responseChecksum!=$checksum_value)
{
		...
		header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?errorMsg=3.Your transaction is denied. Status:CHECKSUM FAILED");
...
else if($response[14] == '0300' )
{
	...
	if ($stmt = $mysqli->prepare("SELECT email, appno, amount, paymentID, response, txnRef, txnTranID, txnError, trackIdResponse from admissions_onlinetransactions where trackID = ?")) {
		...
		if(
			...
			header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?errorMsg=4.Your transaction is denied. Status:Suspicious");
		...
		else
		{
			...
			{    
				...
				{   			
					...
					header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?successMsg=5.Your transaction is Successfull 5. Status:".$ResResult);
...
else 
{
	...
	header("Location:http://msitprogram.net/admissions/paymentAdmissions.php?errorMsg=6.Your transaction is denied. Status:".$ResResult);
