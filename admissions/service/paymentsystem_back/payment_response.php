...
else
{
	...
	header("Location:http://msitprogram.net/admissions/payment.php?errorMsg=Payment Failed.Please try again.");
}
...
if($responseChecksum!=$checksum_value)
{
		...
		header("Location:http://msitprogram.net/admissions/payment.php?errorMsg=Your transaction is denied. Status:CHECKSUM FAILED");
} /////////////////////////////checksum check end
else if($response[14] == '0300' )
{
	...
	if ($stmt = $mysqli->prepare("SELECT email, appno, amount, paymentID, response, txnRef, txnTranID, txnError, trackIdResponse from onlineTransactions where trackID = ?")) {
		...
		if(
			...
			header("Location:http://msitprogram.net/admissions/payment.php?errorMsg=Your transaction is denied. Status:Suspicious");
		}
		else
		{
