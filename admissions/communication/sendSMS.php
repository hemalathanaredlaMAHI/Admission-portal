...
if ($stmt = $mysqli->prepare("SELECT mobile_no FROM ma_user_profile WHERE email = ?")) { 
      ...
      $stmt->fetch();
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, "username=msitad&password=12838257&source=MSITAD&dmobile=".$mobileno."&message=Your Application Number for MSIT Admissions 2015 is ".$appno);
