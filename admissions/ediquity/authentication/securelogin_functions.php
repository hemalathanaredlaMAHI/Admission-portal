...
function sec_session_start() {
        $session_name = 'msitexternal'; // Set a custom session name
...
{
   if ($stmt = $mysqli->prepare("SELECT username, password, salt, usertype, status FROM externalUsers WHERE username = ? LIMIT 1")) { 
      $stmt->bind_param('s', $username); 
      $stmt->execute();
      $stmt->store_result();
      $stmt->bind_result($username, $db_password, $salt, $usertype, $status); 
      $stmt->fetch();
...
function login_check($mysqli) 
{
   if(isset( $_SESSION['username'], $_SESSION['login_string'])) 
   {
     ...
     if ($stmt = $mysqli->prepare("SELECT password FROM externalUsers WHERE username = ?")) 
	 { 
