...
{
   if ($stmt = $mysqli->prepare("SELECT username, password, salt, status, photo_status, profileupdate, educationdetails_status, application_status, payment_status, payment_type FROM ma_users WHERE email = ? and usertype = ? LIMIT 1")) { 
      $stmt->bind_param('ss', $email,$usertype); 
      $stmt->execute();
      $stmt->store_result();
      $stmt->bind_result($username, $db_password, $salt, $status, $photo_status, $profileupdate, $educationdetails_status, $application_status, $payment_status, $payment_type); 
      $stmt->fetch();
	  ...
	  { 
		 ...
		 { 
			   ...
			   $_SESSION['payment_type'] = $payment_type;
				...
				{
					if ($insert_stmt = $mysqli->prepare("select appno from ma_user_gat_exam_details where email = ?")) {   			
					$insert_stmt->bind_param('s', $email); 
					$insert_stmt->execute();
					$insert_stmt->store_result();
					$insert_stmt->bind_result($gatappno); 
					$insert_stmt->fetch();
					if($gatappno!=null)
						$_SESSION['gat_application_no'] = $gatappno;
					$insert_stmt->close();
					}
				}
               return true;    
