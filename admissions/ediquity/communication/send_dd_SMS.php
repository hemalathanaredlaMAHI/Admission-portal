...
{ 
	...
	{ 
		...
		//	$mobileno ="918179150987";
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, "username=msitad&password=12838257&source=MSITAD&dmobile=".$mobileno."&message=Congratulations Applicant, your DD has been received and verified");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
			$data = curl_exec($ch);
			echo "<script>window.location.assign('send_dd_email.php?email=$email');</script>";
			
	}
	else
	{
		echo "<script>alert('Failed to send SMS');window.location.assign('/admissions2014/admin/verify_dd.php');</script>";
	}
}
?>