...
$result = mysql_query("SELECT * FROM  ma_dd_details WHERE appno NOT IN (SELECT appno FROM ma_dd_verification) and appno not in (select appno from ma_dd_sms_pending)");
while($row = mysql_fetch_array($result))
  {
		...
		while($row1 = mysql_fetch_array($result1))
		{
		  ...
		  // $mobileno = "9676555091";
			...
			$data = curl_exec($ch);
			$sql="INSERT INTO ma_dd_sms_pending(appno,dd_phno)VALUES('$appno', '$mobileno')";
		   $result2=mysql_query($sql);
			
	}
				echo "<script>alert('Successfully sent');window.location.assign('/admissions2014/admin/alert_DD_pending.php');</script>";
?>