...
<div class="widget-box">	
	<div class="widget-body">
		<div class="widget-main">
			...
			<?php
				$flag = 0;
				if ($stmt = $mysqli->prepare("SELECT appno, appType, trackID, amount, paymentID, response, txnTranID, transDate, trackIdResponse from onlineTransactions WHERE email = ?")) 
				{ 
			...
			<?php
				/*$flag = 0;
				//amount column added ...by RB on Dec 9,2017
				if ($stmt = $mysqli->prepare("SELECT appno, ddno, bank_name, bank_branch,amount, issue_date from ddTransactions WHERE email = ?")) 
				{ 
					$stmt->bind_param('s', $_SESSION['email']); 
					$stmt->execute();
					$stmt->bind_result($appno, $ddno, $bankname, $bankbranch,$amount, $issuedate); 
					while($stmt->fetch())
					{
						$flag=1;
						echo "<tr>";
