<?php include('backendaccesscheck.php');
sec_session_start();
 ...
 function validateAndSave($file) {
		...
		)
	 {
		...
		$directory = "../authentication/profile_images/";
		$image_ext_name = preg_replace('/[^\w\.\- ]/', '', $_SESSION["offlineemail"]);
		$save_path = $directory.$image_ext_name;

		if($file["type"] == "image/jpg")
		{
			$profile_image_name = $image_ext_name."-profile.jpg";
			$thumb_path = $save_path."-thumb.jpg";
			$profile_path = $save_path.'-profile.jpg';
			$save_path = $save_path.".jpg";
		}
		elseif($file["type"] == "image/jpeg" || $file["type"] == "image/pjpeg")
		{
			$profile_image_name = $image_ext_name."-profile.jpeg";
			$thumb_path = $save_path."-thumb.jpeg";
			$profile_path = $save_path.'-profile.jpeg';
			$save_path = $save_path.".jpeg";
		}
		else
		{
			$profile_image_name = $image_ext_name."-profile.png";
			$thumb_path = $save_path."-thumb.png";
			$profile_path = $save_path.'-profile.png';
		...
		if(
			...
			!resize($save_path, $thumb_path, 150, FALSE) 
			OR
			!resize($save_path, $profile_path, 150, 200) 
		  )
		{
			$result['status'] = 'ERR';
			$result['message'] = 'Unable to save file!'.$save_path."==".$file["type"];
		}

		else {
				$mysqli = new mysqli("45.79.134.238", "msitprogram", "v3NXLXC5YjFPjmH7", "msitprog_admissions");
				if ($insert_stmt = $mysqli->prepare("update ma_user_profile set image_url = ? where email = ?")) 
				{   			
					$insert_stmt->bind_param('ss', $profile_image_name, $_SESSION['offlineemail']); 
					$insert_stmt->execute();
					$insert_stmt->close();
				
						$photo_status = "yes";
						if ($insert_stmt = $mysqli->prepare("update ma_users set photo_status = ? where email = ? ")) 
						{   			
							$insert_stmt->bind_param('ss', $photo_status, $_SESSION['offlineemail'] ); 
							$insert_stmt->execute();
							$insert_stmt->close();
						}
						$_SESSION['offlinephoto_status'] = $photo_status;
						$thumb_pic_url = str_replace("-profile","-thumb",$profile_image_name);
						$_SESSION['offlinethumb_pic_url'] = $thumb_pic_url;
				}
			...
			$result['url'] = 'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME']).'/'.$profile_path;
		}
	 }
