...
<?php
		$qry = "Select * from gat_call_letters limit 1";
		...
		while($temp = $result->fetch_object())
		{
			...
			$pdf->SetFont('times','',11);
			$pdf->SetXY(40,107);
			$pdf->Cell('','','G Narayanamma Institute of Technology & Science,','','','');


			$pdf->SetXY(40,112);
			$pdf->Cell('','','Seminar Hall, F Block, Shaikpet, Mehdipatnam ,','','','');

			$pdf->SetXY(40,117);
			$pdf->Cell('','','(Hi-Tech city road ), Hyderabad-500008 ,','','','');
