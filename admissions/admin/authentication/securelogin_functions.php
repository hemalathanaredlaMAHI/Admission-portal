...
function sec_session_start() {
        $session_name = 'msitadmin'; // Set a custom session name
...
{
   if ($stmt = $mysqli->prepare("SELECT username, password, salt, status, phone_no, usertype FROM ma_admin_users WHERE email = ?")) { 
      $stmt->bind_param('s', $email); 
      $stmt->execute();
      $stmt->store_result();
      $stmt->bind_result($username, $db_password, $salt, $status, $mobile_no, $usertype); 
      $stmt->fetch();
...
function login_check($mysqli) 
{
   if(isset($_SESSION['email'], $_SESSION['username'], $_SESSION['login_string'])) 
   {
     ...
     if ($stmt = $mysqli->prepare("SELECT password FROM ma_admin_users WHERE email = ? and usertype = ?")) 
	 { 
