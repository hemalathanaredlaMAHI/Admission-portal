	...
	if (($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/png") && ($_FILES["file"]["size"] < 2000000) && in_array($upload_exts, $file_exts))
	{
		...
		else
		{
			...
			{*/
				...
				if ($insert_stmt = $mysqli->prepare("update ".prefix."ma_user_profile set image_url = ? where email = ?")) {   			
				...
				{
					$photo_status = "yes";
					if ($insert_stmt = $mysqli->prepare("update ".prefix."ma_users set photo_status = ? where email = ? ")) {   			
